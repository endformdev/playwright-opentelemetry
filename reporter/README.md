...
page (`browser.page.id`). Page IDs are generated by the reporter, starting at
`page-0` for each test tracker. Reloads also advance the document index.

SPA navigation spans (`browser.route`) carry only their local
`browser.route.navigation.index`; the document index stays on the parent span.
